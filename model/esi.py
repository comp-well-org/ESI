###### CODE ADAPTED FROM https://github.com/lucidrains/coca-pytorch ######
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context):
        ...
        # print(q.shape, context.shape, k.shape)
        sim = einsum('b h i d, b j d -> b h i j', q, k)
...
class ESI(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.img_token_head = nn.Linear(image_dim, dim)
        ...
        if self.pretrained_text_encoder:
            self.unimodal_layers = pretrained_text_encoder
        else:
    ...
    def forward(
        ...
        if return_loss and not exists(labels):
            text, labels = text[:, :-1], text[:, 2:]

        if self.pretrained_text_encoder:
            outputs = self.unimodal_layers(text, attention_mask[:, :-1])
        ...
        image_embeds = self.embed_image(images=images)
        image_embeds = self.img_token_head(image_embeds)
        image_embeds = self.img_attn_pool_norm(image_embeds)
        image_tokens = repeat(image_embeds, "n d -> n t d", t=text_tokens.shape[1])
        ...
        return caption_loss + contrastive_loss, caption_loss, contrastive_loss